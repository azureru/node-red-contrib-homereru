// The code is taken from Centrifugo Web client
// https://github.com/centrifugal/centrifuge-js/blob/master/centrifuge.js
// and modified accordingly..
//
// Since there's no proper Node NPM module for centrifugo WebSocket - so
// this code is a barely working modification from there :)
...
centrifugeProto._setupTransport = function() {
	...
	if (!this._websocketSupported()) {
		this._debug("No Websocket support , can't connect");
		return;
	}

	this._transport = new WebSocket(this._rawWebsocketEndpoint());

	this._transport.onopen = function open() {
...
centrifugeProto._disconnect = function (reason, shouldReconnect) {
	...
	if (!this.isDisconnected()) {
		...
		if (this._reconnecting === false) {
			this.emit('disconnect', [disconnectContext]);
		}
	}
...
centrifugeProto._connectResponse = function (message) {
	...
	if (!errorExists(message)) {
		...
		this._restartPing();
		this.emit('connect', [connectContext]);
	} else {
...
subProto._triggerUnsubscribe = function() {
	...
	this.emit("unsubscribe", [unsubscribeContext]);
};
